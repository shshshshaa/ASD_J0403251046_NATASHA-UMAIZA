...
def shellSort(angka):
    ...
    jarak = len(angka) // 2
    # Selama gap masih lebih dari 0 maka proses sorting terus berjalan
    while jarak > 0:
        # Melakukan proses insertion sort pada setiap sublist
        for awal in range(jarak):
            gapInsertionSort(angka, awal, jarak)
        # Setelah satu tahap selesai, gap diperkecil lagi
        jarak = jarak // 2


def gapInsertionSort(angka, mulai, jarak):
    
    # Loop untuk menelusuri elemen sesuai jarak (gap)
    for i in range(mulai + jarak, len(angka), jarak):
        # Menyimpan nilai yang sedang dibandingkan
        nilai_sekarang = angka[i]
        # Menyimpan posisi awal
        posisi = i
        # Selama posisi masih valid dan elemen sebelumnya lebih besar
